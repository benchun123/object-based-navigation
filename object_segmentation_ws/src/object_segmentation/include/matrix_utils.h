...
float IoUCal(const Eigen::Vector4f& box1, const Eigen::Vector4f& box2)
{   
    // this IoU is calculate for world coordinate system
   float left = max(box1[0],box2[0]);  
   float right = min(box1[2],box2[2]);  
   float bottom = max(box1[3],box2[3]);
   float top = min(box1[1],box2[1]);
    if(left>=right || bottom>=top) // if no overlap
        return 0;
    float over_area = (right-left) * (top-bottom);  // overlap area
    // cout << "over area: " << over_area << endl;
    float area_a = (box1[2] - box1[0]) * (box1[1] - box1[3]);
    float area_b = (box2[2] - box2[0]) * (box2[1] - box2[3]);
    // cout << "area a b: " << area_a << " " <<area_b << endl;
