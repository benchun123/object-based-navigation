...
public:
    ...
    int mnClass;
    Eigen::Vector4f bbox;  // image detect bbox
    Eigen::Vector3f loc;  
    Eigen::Vector3f rpy;      
    Eigen::Vector3f dim;
    Eigen::Vector4f bbox_projected; // object project bbox ; left top, right down   

    cv::Mat hsv;
    bool find_asso;   
    bool asso_bad;  // frame object is associtae with map object, but has big distance error 
    int asso_id;

	pcl::PointCloud<pcl::PointXYZRGB> points; // convert from depth image
	std::vector<pcl::PointCloud<pcl::PointXYZRGB> > points_list; // convert from depth image
